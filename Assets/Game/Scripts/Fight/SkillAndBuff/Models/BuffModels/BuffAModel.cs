...
using UnityEngine;
    ...
    public class BuffAModel
    {
        ...
        {
            BuffAConfig config = (BuffAConfig) ExcelDataMgr.Instance.GetConfigData<BuffAConfig>(node.buffId.ToString());
            if (config == null)
            {
                return;
            }
        ...
        {
            BuffAConfig config = (BuffAConfig)ExcelDataMgr.Instance.GetConfigData<BuffAConfig>(buffId.ToString());
            if (config == null)
            {
                return;
            }
