...
public class MovieClip : MonoBehaviour
{
    ...
    public void Init(Texture2D tex, int row, int col, float playTimes = 0)
    {
        if (tex == null) {
            return;
        }
        ...
        // 这里有性能问题，动态创建角色的时候，会导致卡死,所以提前创建出来
        ...
        this._bitmapArr = MovieClipSpriteMgr.Instance.Get(this.texture.name);
